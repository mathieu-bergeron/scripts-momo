...
GROUPE = int(INPUT_PATH.split('_')[1].replace("groupe",""))
HAUTEUR_LIGNE=34
try:
    HAUTEUR_LIGNE=sys.argv[2]
except:
    pass
...
def rangee(out, numero, prenom, initiales, commit, si_couleur):
    if si_couleur:
        style="style='background-color:lightgray;' bgcolor='#DCDCDC'"
    else:
        style=""

    #prenom = prenom.replace("-", u'—'); # non-breaking hyphen
    ...
    with codecs.open(OUTPUT_PATH, 'w', encoding='utf8') as output_file:
        ...
        with codecs.open(INPUT_PATH, encoding='utf8') as input_file:
            ...
            for (i, line) in enumerate(input_file):
                ...
                if 'html' in OUTPUT_PATH:
                    rangee(output_file, numero, name, initiales, commit, linenum%2==0)
